...
R = 5;      % 测量过程噪声
T = 10;     % 测量时间
theta = pi/T;       % 旋转角度       角度增量
distance = 80/T;    % 每次走的距离   距离增量
WorldSize = 100;    % 世界大小       总距离
X = zeros(2, T);    % 存储系统状态
...
%初始化粒子群
for i = 1 : N
    P(:, i) = [WorldSize*rand; WorldSize*rand]; % 在数据范围内产生 随机粒子数据  坐标
...
%% 重采样（更新）
    for i = 1 : N
        ...
        index = randi(N, 1);
        while(wmax > w(index))     % 对应较小权重的粒子
            wmax = wmax - w(index);% 减小权重限制
            index = index + 1;     % 用下一个粒子替换     
            if index > N
                index = 1;         % 最后一个 用第一个替换
            end          
        end
